...
class FiatRate(unittest.TestCase):
    ...
    def test_commands_with_rate(self, mock_get):
        ...
        self.assertIn('€', self.ln.info())
        self.ln.uri()
        self.ln.add()
        self.ln.add('123')
        self.ln.add('0.001')
        self.ln.add('7')
        self.assertIsInstance(self.ln.add('6.7€'), tuple)
        self.assertIsInstance(self.ln.add('6.8e'), tuple)
        self.assertIsInstance(self.ln.add('6.9E'), tuple)
        self.assertIn('€', self.ln.balance())
        self.assertIsInstance(self.ln.channels(pending=False), list)
        self.assertIsInstance(self.ln.channels(pending=True), list)
        self.assertIsInstance(self.ln.pending(), list)
        self.assertIn('€', self.ln.chs())
        self.ln.feereport()
        self.ln.is_pay_req(PAY_REQ)
        self.ln.address().startswith('bc1')
    ...
    def test_commands_without_rate(self, mock_get):
        ...
        self.assertNotIn('€', self.ln.info())
        self.ln.uri()
        self.ln.add()
        self.ln.add('123')
        self.ln.add('0.001')
        self.ln.add('7')
        self.assertIn('No fiat rate available', self.ln.add('6.7€'))
        self.assertIn('No fiat rate available', self.ln.add('6.8e'))
        self.assertIn('No fiat rate available', self.ln.add('6.9E'))
        self.assertNotIn('€', self.ln.balance())
        self.assertIsInstance(self.ln.channels(pending=False), list)
        self.assertIsInstance(self.ln.channels(pending=True), list)
        self.assertIsInstance(self.ln.pending(), list)
        self.assertNotIn('€', self.ln.chs())
        self.ln.feereport()
        self.ln.is_pay_req(PAY_REQ)
        self.ln.address().startswith('bc1')
