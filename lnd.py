...
class Lncli:
    ...
    def balance(self):
        ...
        for key in wallet:
            desc = key.replace('_', ' ')
            rows.append(
                '%s: %s' % (desc, to_btc_str(wallet[key])))
            if show_fiat and int(wallet[key]):
                try:
                    rows[-1] += ' [%s]' % (self.fiat.to_fiat_str(int(wallet[key]), ))
                except RateError:
                    show_fiat = False
        rows.append('Channel')
        for key in channel:
            desc = key.replace('_', ' ')
            rows.append(
                '%s: %s' % (desc, to_btc_str(channel[key])))
            if show_fiat and int(channel[key]):
                try:
                    rows[-1] += ' [%s]' % (self.fiat.to_fiat_str(int(channel[key]), ))
                except RateError:
                    show_fiat = False
        return '\n'.join(rows)
    ...
    def chs(self):
        ...
        for ch in sorted(chs, key=lambda x: x['private']):
            ...
            if show_fiat:
                try:
                    capacity_eur = ' [%s]' % (self.fiat.to_fiat_str(int(ch['capacity']), ))
                except RateError:
                    show_fiat = False

            rows.append('%s%s %s %s%s' % (active, private, self._alias(pubkey), capacity, capacity_eur))
        return '\n'.join(rows)
